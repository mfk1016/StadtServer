...
public class YggdrasilGenerator extends TreeGenerator {
    ...
    public YggdrasilGenerator(Block nwBase) {
    ...
    public void generateTree() {
        ...
        minBranch = Math.max(trunkHeight / 5, 6);
        ...
        for (var locbranch : branchBases) {
            double yUp = Math.max(90D - ((currBranch - 1) * 2.5), 0D);
            double yDown = Math.max(yUp - 2.5, 0D);
            int length = branchLength + 1 + StadtServer.RANDOM.nextInt(3);
            ...
            for (int i = 0; i < branchLogs.size(); i++) {
                ...
                if (i < (branchLogs.size() / 3))
                    continue;
                generateSphereLeaves(log, Math.min(i / 5, 5));
            }
            currBranch++;
        }
        // roots
